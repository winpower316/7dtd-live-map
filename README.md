...
車両・ドローンの所有者はModの`webentityowners`がプレイヤーセーブの所有エンティティ一覧を1分キャッシュで読み、エンティティIDと表示名だけをエージェントへ返します。プラットフォームID、インベントリ、耐久値は送信しません。Mod DLLが実行中プロセスより新しい場合、エージェントはコマンドを呼ばず、次回ゲーム再起動まで所有者なしで安全に継続します。
