...
プレイヤー位置、所有者名付きの車両・ドローン、トレーダー、寝袋、クエスト、全員共有ウェイポイント、バイオーム、サーバー状態、絞り込み済み履歴を合い言葉認証後に表示できます。任意の管理コマンドをWebから実行する機能はありません。
...
- 保存プロフィールのない一時的なプレイヤー名を次のスナップショットで自動整理
- チャット、参加、退出、死亡だけを保持するサーバー履歴
- Windowsエージェントによる10秒間隔の安全なデータ収集
- 読み取り専用の統計・所有者コマンドと全員共有地点を提供するサーバー側Mod
- 既定OFFの5分遅延再起動予約
...
- 所有者名付きの車両・ドローン、補給物資、トレーダー、寝袋、クエスト、共有地点
- バイオームPNG
- FPS、メモリ、エンティティ数などの統計

平日・休日など独自モード名を使う場合は`-ScheduleMode`、実効日長を上書きする場合は`-DayNightLengthMinutes`を指定します。

車両・ドローンの所有者はModの`webentityowners`がゲームの車両・ドローン管理情報にある所有プレイヤーIDを永続プレイヤー一覧の表示名へ解決し、1分キャッシュでエンティティIDと表示名だけをエージェントへ返します。ドローンについてはプレイヤーセーブの所有エンティティ一覧もフォールバックとして参照します。プラットフォームID、インベントリ、耐久値は送信しません。Mod DLLが実行中プロセスより新しい場合、エージェントはコマンドを呼ばず、次回ゲーム再起動まで所有者なしで安全に継続します。
