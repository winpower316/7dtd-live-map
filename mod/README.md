...
IPアドレス、プラットフォームID、インベントリなどは出力しない。所有者一覧は車両・ドローン管理情報の所有プレイヤーIDを永続プレイヤー一覧の表示名へ解決し、ドローンについてはプレイヤーセーブもフォールバックとして参照する。負荷を抑えるため結果は1分間キャッシュする。コマンドはTelnet経由でWindowsエージェントだけが呼び出し、公開Webから任意コマンドを実行する経路は作らない。
